...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, batch_norm=True, activation=nn.ReLU()):
...
class Classifier(nn.Module):
    ...
    def __init__(self, in_features, middle_features=128, out_features=10, n_hidden_layers=1):
...
class Net3Conv(nn.Module):
    def __init__(self):
        ...
        backbone_layers.append(nn.MaxPool2d(kernel_size=2))
        backbone_layers.append(ConvBlock(in_channels=32, out_channels=32, activation=nn.Identity()))

        self.backbone = nn.Sequential(*backbone_layers)
        self.classifier = Classifier(in_features=32 * 7 * 7)
